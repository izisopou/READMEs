...
python python/BinnedFit_4J.py -c config/fourjet_alpha1_multipdf_3_func.config -l 137000 rootfiles/Data_Histo.root -b PFDijet2017_4J -d output/ --fit-spectrum

.config is where you determine the range of fit, initial parameters etc (you will need different configs for each alpha slice so that the parameters are uncorrelated)
...
This step produces the datacard (one .txt and one .root file, .txt is the datacard) using the DijetFitResults_PFDijet2017_4J.root from the previous step, and the Data and Signal 1GeV histos that are inside the respective root files. --xsec is the Reference cross section. The config file is the same as the one when doing the background only fit.
